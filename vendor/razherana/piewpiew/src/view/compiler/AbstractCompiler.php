...
abstract class AbstractCompiler
{
  ...
  final protected function write_map($file): string
  {
    ...
    // If not create it
    if (!is_dir($full_map_dir = trim(rtrim($view->read_cached_config('map'), "/"), '\\')))
  ...
  {
    $view = $this->view;
    return trim(rtrim($view->read_cached_config('map'), '/'), '\\') . DIRECTORY_SEPARATOR . $this->get_compiler_name() . ".json";
  }
  ...
  {
    return trim(rtrim($this->view->read_cached_config('compiled'), '/'), '\\');
  }
